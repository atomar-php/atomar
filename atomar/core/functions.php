...
/**
 * Get the value of a stored variable. If no variable is found, but a default value is provided
...
function variable_get($name, $default = null) {
    $s = R::findOne('setting', 'name=?', array($name));
    if ($s) {
        return $s->value;
    } elseif ($default !== null) {
        // create setting with default value
        $s = R::dispense('setting');
        $s->name = $name;
        $s->value = $default;
        store($s);
        return $default;
...
/**
 * Set the value of a stored variable. Setting the variable to null  or leaving the value field blank will delete the variable.
...
function variable_set($name, $value = null) {
    $s = R::findOne('setting', 'name=? LIMIT 1', array($name));
    if ($value == null) {
 ...
 * @return string the value of the variable or the default value if specified otherwise null.
 */
function system_get($name, $default = null) {
 ...
 * @return boolean will return true or false if the variable was successfully created or deleted.
 */
function system_set($name, $value = null) {
 ...
 * @param string $subject the subect of the email letter
 * @param string $body the body of hte email
 * @param array $options additional options to customize the email including the template to use and any variables to use in the template.
 * @param boolean $debug enables debug mode
 * @return bool
 */
function email($recipient, $subject, $body, $options = null, $debug = false) {
    // NOTE: if you get an error like "Language string failed to load: tls" 
    // make sure you are using the correct port. For gmail: SSL = 465 TLS = 587
    if (is_array($options)) {
        if (!isset($options['template'])) {
            $options['template'] = 'default.html';
        }
...
HTTP;
    ...
    // add system options to all email templates.
    $options['sys'] = \atomar\Atomar::$config;
    ...
    if (\atomar\Atomar::$config['debug']) {
        $twig = new Twig_Environment($loader);
    } else {
        $twig = new Twig_Environment($loader, array(
            'cache' => \atomar\Atomar::$config['cache'] . 'twig',
        ));
    }
    $message = $twig->render($options['template'], $options);
    ...
    $mail = new PHPMailer();
    $mail->IsSMTP();
    $debug_email = system_get('debug_email', '0') == '1';
    if ($debug_email) {
        $mail->SMTPDebug = 1;
    }
    if (\atomar\Atomar::$config['email']['port'] != '') {
        $mail->Port = \atomar\Atomar::$config['email']['port'];
    }
    $mail->Host = \atomar\Atomar::$config['email']['system_email_host'];
    $mail->SMTPAuth = true;
    $mail->Username = \atomar\Atomar::$config['email']['system_email_username'];
    $mail->Password = \atomar\Atomar::$config['email']['system_email_password'];
    if (\atomar\Atomar::$config['email']['secure']) {
        $mail->SMTPSecure = 'tls';
    }
    $mail->SetFrom(\atomar\Atomar::$config['email']['system_email'], \atomar\Atomar::$config['email']['system_email_user_name']);
    $mail->AddReplyTo(\atomar\Atomar::$config['email']['system_email'], \atomar\Atomar::$config['email']['system_email_user_name']);
    $num_emails = 0;
    ...
    $response = $mail->Send();
    if ($debug_email) {
        Logger::log_notice('email send status', $response);
        exit;
    }
    return $response;
}
